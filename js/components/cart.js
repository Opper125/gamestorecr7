 ...
 * Stored in localStorage. Checkout: payment method, location, input tables, promo, transaction refs.
 */
(function() {
'use strict';
const CartManager = {
  ...
  openCart() {
    if (!Auth.isLoggedIn()) { alert('Please log in first'); return; }
  ...
  async openCheckout() {
    ...
    try {
      ...
      if (data && data.length > 0) {
        paymentHtml = data.map(p => `<div class="payment-admin-card" style="cursor:pointer;margin-bottom:0.5rem;" data-id="${p.id}">
          <img src="${p.icon_url||'/assets/icons/default-category.svg'}" alt="" class="payment-admin-icon" />
          <div class="payment-admin-info"><div class="payment-admin-name">${Utils.escapeHtml(p.pay_name)}</div></div>
        </div>`).join('');
      }
    } catch(e) {}
    ...
    content.innerHTML = `
      <h4 style="margin-bottom:0.75rem;">Select Payment Method</h4>
      <div id="checkout-payments">${paymentHtml}</div>
      <div id="checkout-location" style="margin-top:1rem;">
        <h4 style="margin-bottom:0.5rem;">Delivery Location</h4>
        <div class="form-group"><input type="text" class="form-input" placeholder="Full address" id="checkout-address" /></div>
      </div>
      <div style="margin-top:1rem;padding-top:0.75rem;border-top:1px solid var(--stroke-light);">
        ...
        <button id="checkout-submit" class="btn-primary btn-full mt-4">Place Order</button>
      </div>
    `;
    document.getElementById('checkout-submit')?.addEventListener('click', () => {
      alert('Order placed! (Would create order in DB)');
      this.items = [];
      this.save();
      this.closeCart();
