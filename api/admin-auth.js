  ...
  try {
    ...
    switch (action) {
      case 'login': {
        ...
        if (!loginPwd) return res.status(500).json({ error: 'Admin login not configured' });
        if (!password) return res.status(400).json({ error: 'Missing password' });
        if (password !== loginPwd) return res.status(401).json({ error: 'Invalid password', success: false });
      ...
      case 'approve': {
        ...
        if (!approvePwd) return res.status(500).json({ error: 'Admin approve password not configured' });
        if (!password) return res.status(400).json({ error: 'Missing password' });
        if (password !== approvePwd) return res.status(401).json({ error: 'Invalid approve password', success: false });
      ...
      case 'verify_ip': {
        ...
        if (!adminIp) {
          ...
          return res.status(200).json({ configured: false, clientIp: clientIp.split(',')[0].trim() });
        }
        return res.status(200).json({ configured: true, allowedIp: adminIp });
      }
