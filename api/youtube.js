...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (video_id) {
      ...
      if (!response.ok) {
        return res.status(response.status).json({ error: result.error?.message || 'YouTube API error' });
      }
    ...
    if (type === 'videos' || type === 'shorts') {
      ...
      if (!uploadsPlaylistId) {
        return res.status(404).json({ error: 'Upload playlist not found' });
      }
      ...
      if (page_token) {
        playlistUrl += `&pageToken=${encodeURIComponent(page_token)}`;
      }
      ...
      if (!playlistRes.ok) {
        return res.status(playlistRes.status).json({ error: playlistResult.error?.message || 'YouTube API error' });
      }
      ...
      return res.status(200).json({
        data: {
          items,
          nextPageToken: playlistResult.nextPageToken || null,
          prevPageToken: playlistResult.prevPageToken || null,
        },
      });
    }

    // Channel posts / community posts
    if (type === 'posts') {
      // YouTube API doesn't have a direct community posts endpoint via API v3
      // We'll return an empty set with a note, or use a workaround
      return res.status(200).json({
        data: {
          items: [],
          note: 'Community posts not available via YouTube Data API v3. Use YouTube Data API (scraping) for posts.',
        },
      });
    }
