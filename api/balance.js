  ...
  try {
    ...
    switch (action) {
      ...
      case 'debit': {
        ...
        const { error } = await supabase.rpc('debit_user_balance', {
          p_user_id: user_id,
          p_amount: parseFloat(amount),
          p_note: note || null,
        });
      ...
      case 'approve_deposit': {
        if (!deposit_id) {
          return res.status(400).json({ error: 'Missing deposit_id' });
        }
      ...
      case 'refund_order': {
        ...
        const { error } = await supabase.rpc('refund_g2bulk_order', {
          p_order_id: order_id,
        });
      ...
      case 'get_balance': {
        ...
        const { data, error } = await supabase
          .from('users')
          .select('id, username, game_balance')
          .eq('id', user_id)
          .single();
